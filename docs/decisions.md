...
## 2026-09-19 night (2) · length is training-only shaping; the grader is length-free (lead)
- The grader's `reward = correctness × efficiency` no longer sees answer length at all; `gates.length_factor` (min(1, cap/tokens)) is applied only inside the trainer's group hook to the shaped reward (`length_shaping`, on by default; the held-out evaluator and `evals.run` never apply it). Evals therefore measure content: whether a frontier model can reach ~100 % is a statement about the data and the judge, not about verbosity.
- Verbatim tool-output pasting remains a format gate in the grader (it is not a length question).

## 2026-09-20 early · grounding is a multiplier, not a gate; find_symbol ranges count as shown (lead: "all should reach grading")
- Frontier audit: Opus 5 reached grading on only 88 % of Sonnet's hard tasks; 7 of the 9 misses were grounding, and half of those cited exact `find_symbol` ranges the model had seen in tool output but the env recorded only the signature line as shown. The rest cited ranges partly overlapping what was read.
- Now: `RepoTools.find_symbol` records each displayed `path:Lstart-Lend` as shown (like grep hits; lane A's test updated). The grader computes `citations_grounded` = mean over citations of the share of cited lines shown (±1 line) and multiplies the reward by it; the `grounding` gate fires only when nothing cited was shown. Fabricated files (`citations_exist`) remain a hard gate. The ablation switch `CODEQA_HONESTY_GATES=off` ignores the multiplier as before.
- Effect: Opus reaches grading on 96 % (72/75), correct rate 96 %, mean reward 0.86; the three left are an ambiguous bare filename, a turn-limit stall, and a judge API error.
- Still open (lane A, env): a stalled episode gets no final chance to answer. Proposal: when the tool budget is exhausted, the driver/env makes one more turn with tools refused ("no tool calls remaining; answer now") instead of ending the episode, so "reached grading" no longer depends on the turn cap.

## #9 (2026-09-19 22:13) Phase 1 trains on run one's verifiable set; caps ≥ 20 calls; prompt states limits and the citation form
The full task set (`all.jsonl`, 31 % explain, filtered to the 10–90 % pass band) gave no "answer earlier" gradient: winners hit the turn cap like the losers, half the groups were constant, and held-out after 10 steps was base level (1.7 %) while run one on `run1.jsonl` reached 66 % on the same tasks. Phase 1 therefore trains all arms on `run1.jsonl`; the full set is a later curriculum stage from a checkpoint that already answers. Caps raised to 20/22 (locate, value), 24/26 (enumerate, trace), 30/32 (explain) with context 65536; the prompt now states both limits, the exact `[path:L10-L20]` form, and that citing unseen lines fails. Efficiency pressure comes from the run-two multiplier, not from the caps. Evidence: `data/logs/p1_full_allset`, LOG 22:13.

## #10 (2026-09-19 23:06) Reward pairing by env object, not by index; citation rule first in the prompt
Since `RetryOnFailure` returns trajectories in completion order, pairing rewards by index scrambled credit inside every group (0 % in run one, 35–67 % afterwards). Fixed by pairing through the env object that arrives with each trajectory, guarded by `env/all/pairing_mismatch` (0.0 expected) and `test_pairing.py`. Verdicts drawn from the affected runs (full task set "dead end", 9B drift) are void; phase 1 restarts on 9B/run1.jsonl. The prompt now leads with the exact `[path:L10-L20]` rule and shows the forms that score zero; the grader stays strict so product and training agree.

## #11 (2026-09-20 10:14) Stratified batches and a stronger judge for a readable curve
With 8–12 tasks per step the task-type draw moved train reward by ±0.15 per step and hid the trend; batches are now a proportional interleave over task types (same mix every step), the judge is Sonnet 5 (Haiku's rubric scores were the noisiest reward component), and the shell harness allows `2>/dev/null`. Held-out points every 8 steps plus the 5-step mean in the Workshop are the curve to read; single steps are not.

## #12 (2026-09-20 11:10) The reward mistake, for the talk
R = [correctness × efficiency × grounded_fraction + grounded_credit] × length_factor − 0.1·[no answer], gated to 0 by format, citation-parse, all-citations-exist, zero-grounding and over-budget checks. In phases 3–4 efficiency ≡ 1 (variant "none") and length_factor is 1 below the cap, so no term paid for an additional claim while the existence gate and grounded_fraction made each one a liability; with the token-summed importance-sampling loss the gradient shortened answers. Observed on the 40-task held-out set in both bash and lean arms: grounding 54 → 72 % and 82 → 90 %, calls −20 %, answer length −40 %, rubric coverage on explain questions down (bash 0.46 → 0.34). Run-two fix: graded rubric coverage as the explain correctness term, per-claim citation credit instead of the all-or-nothing existence gate, a length floor mirroring the cap; keep the no-answer penalty and the grounding checks.

## 13. Phase 7: rounds, a token cap instead of a call cap, forced final answer, reward v3 (2026-09-20 15:00)
Decision: for the bash agent, drop the tool-call limit in favour of a context cap (32k prompt tokens) and a message cap (24), let a message carry several commands as one round, tell the model the remaining budget after every round, and inject a tool-free final turn when the budget runs out. Reward v3 puts correctness first: R = c × (0.75 + 0.25·grounded) until step 8, then c × (0.6 + 0.25·grounded + 0.15·efficiency) with efficiency on the true token cost. Hard zeros (no citation, nonexistent path, nothing cited was read) stay; the length floor stays; the forced answer keeps 90 %.
Why: calls are the wrong unit (Tinker bills every turn's prompt again, so cost is the token sum); 20–36 % of early episodes ended at the cap with no answer and taught only "answer sooner"; efficiency in the product from step 1 dilutes the correctness signal; a plain additive reward pays for fast wrong answers, the correctness-gated form does not.
Kept comparable: the held-out eval reports the phase-6 formula whatever the training reward is.
